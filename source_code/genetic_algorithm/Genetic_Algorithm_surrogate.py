...
def evaluate_individual_surrogate(args):
    global surrogate_model

    individual, index = args
    max_value = max(individual)  
    average_value = sum(individual) / len(individual)
    zero_count = individual.count(0)
    median_value = np.median(individual)
    std_value = np.std(individual)
    statistics = [max_value, average_value, zero_count, median_value, std_value]
    # 추가적인 통계 정보 반영하여...
    individual_stat = individual + statistics

    fitness_value = surrogate_model.predict(individual_stat)
    return (index, fitness_value)
...
def fitness_func(population, original_station_df_ref_for_fitness_func, path_history, pool, current_generation_number):
    ...
    if car_paths_df_current_gen is None or car_paths_df_current_gen.empty:
        ...
        return [0] * len(population), population 
...
def fitness_func_for_surrogate(population, pool, current_generation_number):
    try:
        args_list = [(ind, idx) for idx, ind in enumerate(population)]
        map_start_time = time.time()
        calculated_chunksize = max(1, len(population) // CORE_NUM) if CORE_NUM > 0 else 1

        results = list(pool.imap(evaluate_individual_surrogate, args_list, chunksize=calculated_chunksize))
        map_end_time = time.time()
        print(f"  - 멀티프로세싱 시간: {map_end_time - map_start_time:.2f}초")

    except Exception as e:
        print(f"ERROR in fitness_func_for_surrogate: {e}")
    
    if not results:
        print("Warning: pool.imap returned empty results in fitness_func_for_surrogate.")
        return [-np.inf] * len(population), population
...
def genetic_algorithm():
    ...
    for generation in range(GENERATIONS): # 각 세대 루프 시작
        ...
        with Pool(processes=CORE_NUM,
            ...
            if generation % 5 == 0 or generation >= 2000:
                fitness_values, sorted_population = fitness_func(population, original_station_df, path_history, pool, generation)
                print(f"세대 {generation + 1}의 적합도 계산 완료")
            else:
                fitness_values, sorted_population = fitness_func_for_surrogate(population, pool, generation)
            print(f"세대 {generation + 1}의 surrogate 적합도 계산 완료")
            ...
            if generation >= 10:  # 11개 세대(index 10) 이상의 데이터가 쌓여야 비교 가능
                ...
                # 2. 적합도 평균 변화량 체크
                prev_10_fitness = fitness_history[-11:-1] 
                curr_10_fitness = fitness_history[-10:]   
