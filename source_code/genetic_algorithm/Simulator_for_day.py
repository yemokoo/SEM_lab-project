...
class Simulator:
    ...
    def calculate_penalty(self, failed_trucks_df):
        ...
        for station_id, station in self.stations.items():
            number_of_charges += station.num_of_chargers
    ...
    def calculate_of(self):
        ...
        penalty_df = self.calculate_penalty(self.failed_trucks_df)  # 트럭 정지 페널티 및 충전기 초과 설치 페널티
        ...
        ax.bar(x - width, merged_df['capex'] / 10000000, width, label='CAPEX', color='red')
        ax.bar(x, merged_df['opex'] / 10000000, width, label='OPEX', color='orange')
        ax.bar(x + width, merged_df['revenue'] / 10000000, width, label='Revenue', color='blue')

        # 페널티 정보 텍스트로 추가 (그래프 상단)
        ax.text(0.95, 0.95, f"Truck Penalty: {round(truck_penalty / 10000000, 2)}",
                horizontalalignment='right',
                verticalalignment='top',
                transform=ax.transAxes)
        ax.text(0.95, 0.90, f"Charger Penalty: {round(charger_penalty / 10000000, 2)}",
                horizontalalignment='right',
                verticalalignment='top',
                transform=ax.transAxes)
        ...
        ax.set_ylabel('Amount (10 million)')
        ax.set_title('CAPEX, OPEX, and Revenue by Station')
        ax.legend()
