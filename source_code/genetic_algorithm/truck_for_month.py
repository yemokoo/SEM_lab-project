...
class Truck:
    ...
    """
    def __init__(self, path_df, simulating_hours, link_id_to_station, model, links_to_move=40, initial_soc=None):
        # --- 내부 헬퍼 함수 (기존과 동일) ---
        ...
        self.is_respawned = (initial_soc is not None)
    ...
    def step(self, current_time):
        ...
        # --- [수정] 방전 시 리스폰 로직 호출 ---
        if self.SOC <= 0.001:
            # Simulator에 리스폰 처리를 요청한 뒤, 현재 트럭은 정지시킴
            if hasattr(self.model, 'handle_truck_respawn'):
                self.model.handle_truck_respawn(self)
            self.stop()
            return
    ...
    def get_info(self):
        """
        시뮬레이션 종료 시 트럭의 최종 정보를 수집합니다.
        (total_distance_planned를 '다음 주요 휴식지'까지의 거리로 재정의하여 페널티 중복 방지)
        """
        # --- 1. 'total_distance_planned' 값 계산 로직 (수정됨) ---
        total_distance_planned_km = 0.0
        if hasattr(self, 'path_df') and self.path_df is not None and not self.path_df.empty:
            # 기본값은 실제 경로의 총 거리로 설정
            total_distance_planned_km = float(self.path_df['CUMULATIVE_LINK_LENGTH'].iloc[-1])

            # 현재 멈춘 위치 이후의 '85분 이상 휴식 지점'들을 찾음
            final_index = self.current_path_index
            future_significant_stops = [idx for idx in self.significant_stop_indices_set if idx > final_index]

            if future_significant_stops:
                # 만약 다음 휴식 지점이 있다면, 그중 가장 가까운 곳의 누적 거리를 '계획된 운행 거리'로 재정의
                next_stop_index = min(future_significant_stops)
                total_distance_planned_km = float(self.path_df['CUMULATIVE_LINK_LENGTH'].iloc[next_stop_index])
                
        # --- 2. 'traveled_distance_at_last_stop85' 계산 로직 (기존과 동일) ---
        ...
        info_df = pd.DataFrame([{
            ...
            'total_distance_planned': total_distance_planned_km, # 수정된 값이 여기에 할당됨
            'traveled_distance_at_last_stop85': traveled_distance_at_last_stop85_km, # 이 값은 기존 로직을 따름
